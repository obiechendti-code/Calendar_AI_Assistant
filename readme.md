...
**Project Status:** Stable (v3.0) | **Maintainer:** Obie Chen
...
- **專案 A (例如：產品開發)**：識別關鍵字如 `Product`, `App`, `網站`, `上線`。
- **專案 B (例如：行銷活動)**：識別關鍵字如 `Marketing`, `廣告`, `社群`。
...
- **Email**：發送排版精美的 HTML 郵件至指定信箱（支援寄給自己、公司信箱或團隊群組）。
- **Notion (開發中)**：支援將每日匯報同步至 Notion Database，建立長期專案日誌。
...
2. **Google Apps Script 權限**：需允許指令碼讀取您的 Google Calendar 與發送 Gmail。
    - **驗證方式**：首次手動執行 `sendDailyBriefing` 函式時，系統會自動彈出授權視窗，請依照螢幕指示完成授權（若出現「未驗證應用程式」警告，請點擊「進階」並選擇前往即可）。
3. **Google Gemini API Key**：
    - 需至 [Google AI Studio](https://www.google.com/search?q=https://aistudio.google.com/) 申請免費的金鑰。
    - 模型需求：支援 `gemini-2.0-flash-lite` 或以上版本。
        - 截至 2026.01.30 為最低版本，若未來更新可透過 Debug 模組查詢可用模型。

### 選用條件 (Optional)

- **(尚未開發) Notion Integration**：若需同步至 Notion，需準備 Notion Integration Token 與 Database ID。
...
| 層級 (Layer) | 模組名稱 | 職責說明 |
| --- | --- | --- |
| **Workflow** | `Workflow_DailyBrief` | **核心流程**。負責串接 Config、資料獲取、AI 推論與信件派送。 |
| **Config** | `Config` / `Prompt` | **邏輯設定**。管理模型版本、排程規則與 AI 人設 (System Instructions)。 |
| **Utils** | `CalendarParser` | **資料清洗**。處理 Regex 解析與標題標準化。 |
| **Utils** | `TaskClassifier` | **分類邏輯**。維護專案關鍵字映射表 (Mapping Table)。 |
| **Security** | `Setup` / `Properties` | **機密管理**。透過 Script Properties 儲存 ID 與 Key，確保程式碼本身不含敏感資料。 |

## 🚀 如何開始 (Getting Started)

本專案設計為「零程式基礎」也能輕鬆部署。

👉 **請參閱：GAS行事曆秘書：完全安裝手冊 (Manual.md)**
...
👉 **請參閱：[常見問題與故障排除 (FAQ.md)](https://www.google.com/search?q=./FAQ.md)**
...
- **Update Config**: 修改 `Config.gs` 即可調整大部份行為。